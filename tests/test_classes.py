...
class TestProduct:
    ...
    def test_product_zero_quantity_raises_error(self):
        """Тест: создание продукта с нулевым количеством."""
        with pytest.raises(
            ValueError,
    ...
    def test_product_add_method_different_types(self):
        ...
        with pytest.raises(
            TypeError,
            match="Нельзя складывать объекты разных классов"
        ):
...
class TestSmartphone:
    ...
    def test_smartphone_zero_quantity_raises_error(self):
        """Тест: создание смартфона с нулевым количеством."""
        with pytest.raises(
            ValueError,
            match="Товар с нулевым количеством не может быть добавлен"
        ):
            Smartphone(
                "iPhone", "Смартфон", 50000.0, 0,
                "Высокая", "15 Pro", 256, "черный"
            )
...
class TestLawnGrass:
    ...
    def test_lawn_grass_zero_quantity_raises_error(self):
        """Тест: создание газонной травы с нулевым количеством."""
        with pytest.raises(
            ValueError,
            match="Товар с нулевым количеством не может быть добавлен"
        ):
            LawnGrass(
                "Газонная трава", "Описание", 1500.0, 0,
...
class TestCategory:
    ...
    def test_average_price_with_products(self):
        """Тест подсчета средней цены в категории с товарами."""
        products = [
            Product("Товар1", "Описание1", 100.0, 10),
            Product("Товар2", "Описание2", 200.0, 20),
    ...
    def test_average_price_with_one_product(self):
        """Тест подсчета средней цены в категории с одним товаром."""
        products = [Product("Товар", "Описание", 150.0, 5)]
        category = Category("Категория", "Описание", products)
    ...
    def test_add_product_invalid_type(self):
        ...
        with pytest.raises(
            TypeError,
            match="Можно добавлять только объекты Product"
        ):
